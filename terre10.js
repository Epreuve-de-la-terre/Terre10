...
let start = arg
let int = [];
let car; let min; let max; let mil;
let stop = 0; let val = []; let y = 0;
...
while (int.length < 2) {
  ...
  if ((start * start) > arg) {        //Détermine la valeur haute de l'interval
    int.shift()
    int.push(start)
  } else if ((start * start) < arg) { //Détermine la valeur basse de l'interval
    int.unshift(start)
  } else {
...
while (stop < 1) {
  ...
  if (car * car > arg) {            //Ajoute au millieu la moitié de la différence
    ...
    y++
    val[y] = car
  } else if (car * car < arg) {     //Soustrait au miilieu la moitié de la différence
